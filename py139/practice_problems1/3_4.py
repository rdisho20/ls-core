...
def process_transactions(lst, amount):
    filtered = filter(lambda item: item['amount'] > amount, lst)
    print(dict(key: value for item in filtered for key, value in item.items()))
    ...
    return ((item['id'], item['amount']) for item in transactions)
