...
// TODO:
// code starts acting funny @ 't' in 'Blute'
function indexOf(firstStr, secondStr) {
  ...
  for (let mainIndex = 0; mainIndex < secondStr.length; mainIndex++) {
    ...
    for (let index = nextIndex; index < firstStr.length; index++) {
      ...
      console.log(`mainChar: ${mainChar}, char: ${char}`)
      console.log(`mainIndex: ${mainIndex}, index: ${index}`);
      ...
      if (mainChar !== char) {
        ...
        if (consecutiveCharacters === 0) {
          console.log(`Assigning ${index} to 'startingIndex'`);
          startingIndex = index; // set the starting index for potential match
          nextIndex = startingIndex;
        }
        consecutiveCharacters++;
        nextIndex++;
        console.log(`CONSEC CHAR COUNT: ${consecutiveCharacters}`);
        console.log();
